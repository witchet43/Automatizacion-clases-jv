// Compatibilidad operativa del cierre de unidad cuando la siguiente Unidad existe
// en las fuentes operativas/planeacion pero aun no existe como Topic de Classroom.
// Este override conserva la regla oficial: mover solo Tarea/Actividad/Practica/Quiz
// PUBLISHED sin unidad a la siguiente unidad inmediata no cerrada.
function reubicarCourseWorkSinUnidad_(ss, courseId, unidadActual) {
  const currentNo = extraerNumeroUnidad_(unidadActual);
  if (!currentNo) throw new Error('No se pudo identificar el numero de ' + unidadActual + '.');
  ...
  if (!targetNo) {
    throw new Error('No existe una siguiente unidad verificable y no cerrada despues de ' + unidadActual + '.');
  }
