...
function procesarSolicitudesImportacion() {
  ...
  try {
    ...
    try { procesarSolicitudRecalculoFinalUnidadCero_(); }
    catch (directErr) { console.error('Solicitud de recálculo final directo: ' + String(directErr && directErr.message ? directErr.message : directErr)); }

    const lastRow = Math.max(sh.getLastRow(), 1);
    const values = sh.getRange(1, 1, lastRow, 6).getDisplayValues();
    let row = -1;
    for (let i = 1; i < values.length; i++) {
      if (String(values[i][0] || '').trim() === IMPORT_REQUEST.KEY) { row = i + 1; break; }
    }
    ...
    if (estado !== IMPORT_REQUEST.REQUESTED) return {procesado: false, motivo: 'SIN_SOLICITUD_PENDIENTE', estado: estado};

    const targetQuizId = String(sh.getRange(row, 4).getDisplayValue() || '').trim();
    if (!targetQuizId) throw new Error('Falta el Quiz ID objetivo de la importación.');
    ...
    try {
      ...
      let quizRow = -1, q = null;
      for (let i = 1; i < data.length; i++) {
        if (String(data[i][h['Quiz ID']] || '').trim() === targetQuizId) { quizRow = i + 1; q = data[i]; break; }
      }
      if (!q) throw new Error('No se encontró el Quiz ID objetivo: ' + targetQuizId);

      const estadoQuiz = String(q[h['Estado']] || '').trim().toUpperCase();
      if (estadoQuiz !== QUIZ_PIPELINE.CREATED) throw new Error('El Quiz ID ' + targetQuizId + ' no está en estado CREADA; estado actual: ' + estadoQuiz);
      ...
      if (!courseId || !workId || !formId) throw new Error('Faltan IDs de curso, Classroom o Form para ' + targetQuizId);

      const result = procesarCalificacionesQuiz_(courseId, workId, formId, true, targetQuizId);
      const resumen = 'Importación manual: ' + result.actualizadas + ' actualizadas; ' + result.yaCalificadas + ' ya calificadas; ' + result.sinCorrespondencia.length + ' sin correspondencia; ' + result.noTurnedIn + ' no TURNED_IN.' + (result.ajuste ? ' Ajuste aplicado: +' + result.ajuste + ' puntos.' : '');
