/**
 * Adaptador de transporte para crear una ACTIVIDAD EN CLASE mediante el
 * entrypoint canónico crearActividad(). No contiene reglas académicas.
 *
 * Configuración Quizzes:
 * A Clave = SOLICITUD_CREAR_ACTIVIDAD_CLASE
 ...
 */
const ACTIVITY_CLASS_REQUEST = Object.freeze({
  KEY: 'SOLICITUD_CREAR_ACTIVIDAD_CLASE',
  REQUESTED: 'SOLICITAR',
...
function procesarSolicitudCrearActividadClase_() {
  ...
  const row = buscarFilaSolicitud_(sh, ACTIVITY_CLASS_REQUEST.KEY, 8);
  ...
  if (estado !== ACTIVITY_CLASS_REQUEST.REQUESTED) {
  ...
  if (!raw) throw new Error('La solicitud de actividad en clase no contiene JSON de parámetros.');

  let params;
  try { params = JSON.parse(raw); }
  catch (err) { throw new Error('JSON inválido en solicitud de actividad en clase: ' + mensajeErrorOperacion_(err)); }

  sh.getRange(row, 2).setValue(ACTIVITY_CLASS_REQUEST.PROCESSING);
  sh.getRange(row, 6).setValue(new Date());
  SpreadsheetApp.flush();

  try {
    params = resolverCursoActividadClaseSolicitud_(params);
    const result = crearActividad(params);
    ...
    if (resumen.state.toUpperCase() !== 'DRAFT') throw new Error('La actividad de prueba no quedó DRAFT.');
    if (!resumen.workId) throw new Error('La actividad de prueba no devolvió workId.');

    sh.getRange(row, 2).setValue(ACTIVITY_CLASS_REQUEST.DONE);
    ...
    sh.getRange(row, 2).setValue(ACTIVITY_CLASS_REQUEST.ERROR);
...
function resolverCursoActividadClaseSolicitud_(params) {
